...
Thinking can be turned off per request with
...
"""
import argparse, base64, io, json, os, re, sys, time, threading, uuid
...
PORT = 8888
MODEL_LABEL = None  # /v1/models id (set from --model-label)

vision_lock = threading.Lock()
vision_model = None   # lazy-loaded on first image request
...
app_config = None  # exllamav3 Config, set in build_model (vision component source)


def get_vision_model():
    """Lazy-load the vision component on first image request."""
    global vision_model
    with vision_lock:
        if vision_model is None:
            from exllamav3 import Model
            print(" == loading vision component ...", flush=True)
            vm = Model.from_config(app_config, component="vision")
            vm.load()
            vision_model = vm
            print(" == vision component ready", flush=True)
        return vision_model


def load_image(url):
    """OpenAI image_url -> PIL image. Supports data: base64, http(s), file path."""
...
def extract_images(messages):
    """Split OpenAI multimodal messages: image_url parts -> {'type':'image'}
    placeholders (the HF template renders the image token); returns
    (messages, pil_images) with images in document order."""
    images = []
    out = []
    for m in messages:
...
def build_model(argv, use_draft = True):
    ...
    gen_kwargs = {}
    if args.chunk_size:
        # model.load() plans the layer split around this, the generator must not
        # then feed it bigger chunks, so both sides get the same number
        gen_kwargs["max_chunk_size"] = args.chunk_size
    if use_draft:
        model, config, cache, tokenizer, draft_model, draft_config, draft_cache = \
            model_init.init(args, progress = True)
        # DFlash-style block-diffusion drafters emit a FIXED block (block_size - 1)
        # per step and num_draft_tokens only truncates it; asking for more than the
        # block holds crashes in the verify loop with a shape mismatch rather than
        # anything readable. MTP is a real loop count, so it has no such ceiling.
        if args.num_draft_tokens and not draft_model.caps.get("mtp_draft"):
            block = draft_model.caps.get("default_draft_size", 4)
            if args.num_draft_tokens > block:
                sys.exit(
                    f" !! -ndt {args.num_draft_tokens} is too high for this drafter. "
                    f"{draft_config.architecture} emits a fixed block of {block} draft "
                    f"tokens per step and -ndt can only truncate it. Use -ndt <= {block}, "
                    f"or switch to '-dm mtp', whose draft length is a loop count."
                )
        generator = Generator(
            model, cache, tokenizer, **gen_kwargs,
            draft_model = draft_model, draft_cache = draft_cache,
            # num_draft_tokens defaults to the draft model's arch-declared
            # default_draft_size (DFlash2: block_size - 1 = 7, MTP: 4) when -ndt
            # is not given. Must match model_init's max_history sizing, which
            # reads args.num_draft_tokens and the same caps.
        ...
        generator = Generator(model, cache, tokenizer, **gen_kwargs)
    global app_config
    app_config = config
...
def split_reasoning(text, thinking = True):
    ...
    With thinking off the template pre-closes the block, so it is all content.
...
def generate_full(generator, tokenizer, messages, max_tokens, temperature,
    ...
    if images:
        vm = get_vision_model()
        embeds = [vm.get_image_embeddings(tokenizer, img) for img in images]
        import json as _json
        raw_cfg = _json.load(
            open(os.path.join(app_config.directory, "config.json"),
                 encoding = "utf-8"))
        itok = tokenizer.id_to_piece[raw_cfg["image_token_id"]]
        rendered = tokenizer.hf_render_chat_template(
            messages, add_generation_prompt = True, enable_thinking = thinking,
            tools = tools)
        if rendered.count(itok) != len(embeds):
            raise ValueError(
                f"{len(embeds)} image(s) supplied but template has "
                f"{rendered.count(itok)} image placeholder(s)")
        for e in embeds:
            rendered = rendered.replace(itok, e.text_alias, 1)
    ...
    def run_once():
        ...
        job = Job(input_ids = input_ids, max_new_tokens = max_tokens,
                  ...
                  sampler = sampler, seed = seed, embeddings = embeds)
        prefill_seen = 0
        with gen_lock:
    ...
    if calls:
        ...
        finish = {"max_new_tokens": "length", "eos": "stop",
                  "stop_condition": "stop", "banned": "content_filter"}.get(
                      reason, "stop")
    reasoning, content = split_reasoning(content, thinking)
...
async def models(request):
    ...
    return web.json_response({"object": "list", "data": [{
        "id": MODEL_LABEL or "qwen3.8-27b-exl3-4.00bpw-dflash2",
        "object": "model",
        "owned_by": "exl3",
        # thinking is on/off only (reasoning_effort "none"), no graded levels
...
def parse_request(body):
    ...
    if isinstance(stop, str):
        ...
        stop = None
    # Thinking on/off: chat_template_kwargs.enable_thinking (vLLM/llama.cpp
    # style) wins, else reasoning_effort "none" turns it off. Default on.
    kwargs = body.get("chat_template_kwargs")
    if isinstance(kwargs, dict) and isinstance(kwargs.get("enable_thinking"), bool):
        thinking = kwargs["enable_thinking"]
    else:
        thinking = str(body.get("reasoning_effort") or "").lower() != "none"
    return dict(
        ...
        model_id = body.get("model", "qwen3.8-27b-exl3-4.00bpw-dflash2"),
    ), None
...
async def chat_completions(request):
    ...
    async def run():
        ...
        call_idx = [0]
        # generation starts inside <think> (template), unless thinking is off
        in_think = [req["thinking"]]
        THINK_CLOSE = "</think>"
        ...
        while True:
            ...
            if kind == "delta":
                ...
                if req["include_usage"]:
                    # OpenAI stream_options.include_usage: final chunk, empty choices
                    usage = {"id": cid, "object": "chat.completion.chunk",
...
def main():
    ...
    ap.add_argument("-chunk", "--chunk_size", type = int, default = None,
                    help = "Prompt-ingestion chunk in tokens. Default splits the "
                           "difference badly: the loader plans for 4096 but the "
                           "generator only ever feeds 2048, so half the planned "
                           "transient VRAM goes unused. Setting this pins both to "
                           "the same number. Bigger = faster prefill (lower TTFT on "
                           "long prompts) but a larger transient spike during load, "
                           "which at high context can tip the split into OOM")
    ap.add_argument("-ndt", "--num_draft_tokens", type = int, default = None,
                    help = "Draft tokens per step. Default is the drafter's own "
                           "(MTP: 4, DFlash2: 7). Raising it lengthens each "
                           "speculative block: more tokens per target forward when "
                           "the draft is accepted, more wasted work when it is not. "
                           "On this hybrid linear-attention model every +1 costs "
                           "~144 MiB of VRAM (the fp32 recurrent verify history, "
                           "one row per draft token across the 48 GDN layers), so "
                           "check free VRAM before raising it at high context")
    ap.add_argument("-dds", "--dynamic_draft", action = "store_true",
                    help = "Adapt the draft length to the measured acceptance rate, "
                           "with -ndt as the CEILING rather than a fixed length. "
                           "VRAM is still reserved for the full -ndt, but short "
                           "blocks are cut early when the drafter is not confident")
    ap.add_argument("-dc", "--draft_confidence", type = float, default = 0.4,
                    help = "Confidence target for -dds truncation (default 0.4). "
                           "Lower = draft further before giving up")
    ...
    # Only 'mtp' and 'none' are keywords; anything else is read as a directory.
    # Saying so here beats a FileNotFoundError traceback out of Config.from_directory
    # when someone passes a model's name instead of its path.
    if use_draft and not use_mtp and not os.path.isdir(args.draft_model):
        sys.exit(
            f" !! -dm {args.draft_model!r} is not a directory. -dm takes a path to a "
            f"draft model, or one of the keywords 'mtp' (head inside the main "
            f"checkpoint) or 'none' (no drafting). A model's name is not enough - "
            f"for the DFlash2 draft, pass its full path, e.g. "
            f"D:\\qwen38-dflash2\\models\\dflash2-exl3"
        )
    argv = ["-m", args.model,
    ...
    # -ndt/-dds/-dc only exist on model_init's parser when draft args were added
    if use_draft:
        if args.num_draft_tokens:
            argv += ["-ndt", str(args.num_draft_tokens)]
        if args.dynamic_draft:
            argv += ["-dds", "-dc", str(args.draft_confidence)]
    if args.chunk_size:
        argv += ["-chunk_size", str(args.chunk_size)]
    if args.cache_quant:
    ...
    if use_draft:
        print(f" == draft tokens per step: {generator.num_draft_tokens}"
              + (f" (ceiling; dynamic, confidence {args.draft_confidence})"
                 if args.dynamic_draft else " (fixed)"), flush = True)
